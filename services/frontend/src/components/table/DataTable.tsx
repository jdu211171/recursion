...
export default function DataTable<T>({ columns, rows, rowKey, onRowAction, selectedIds, onSelectionChange, rowActions, loading, headerContent, search, onSearchChange }: Props<T>) {
  ...
  return (
    <>
      {/* Controls / filters block above the table surface */}
      {headerContent && (
      ...
      <div className="card" role="region" aria-label="Data table" style={{ boxShadow: 'none', background: 'linear-gradient(180deg, rgba(255,255,255,0.10), rgba(255,255,255,0.04))', backdropFilter: 'blur(10px) saturate(120%)', WebkitBackdropFilter: 'blur(10px) saturate(120%)', padding: 0 }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0 }}>
            ...
            <tr>
              ...
              {columns.map((c) => (
                <th
                  key={String(c.key)}
                  scope="col"
                  style={{ textAlign: 'left', padding: '12px 16px', cursor: c.sortable ? 'pointer' : 'default', userSelect: 'none' }}
                  onClick={() =>
                    c.sortable &&
                    state.setSort(s => ({ key: String(c.key), dir: s.key === String(c.key) && s.dir === 'asc' ? 'desc' : 'asc' }))
                  }
                >
                  {c.header}
                  {c.sortable && state.sort.key === String(c.key) && (state.sort.dir === 'asc' ? ' ▲' : ' ▼')}
                </th>
              ))}
          ...
          <tbody>
            {loading &&
              Array.from({ length: Math.min(10, state.pagination.pageSize) }).map((_, i) => (
                <tr key={`s-${i}`}>
                  <td style={{ padding: '8px 10px', textAlign: 'center', width: 44 }}>
                    <span className="skeleton" style={{ display: 'inline-block', width: 16, height: 16 }} />
                  </td>
                  {columns.map((_, idx) => (
            ...
            {!loading && state.pageRows.length === 0 && (
              <tr>
                <td colSpan={columns.length + 2} style={{ padding: '14px 16px' }} className="muted">
                  No rows
                </td>
              </tr>
            )}
            {!loading &&
              state.pageRows.map(r => {
                const id = rowKey(r)
                return (
                  <tr key={String(id)}>
                    <td style={{ padding: '8px 10px', textAlign: 'center', width: 44 }}>
        ...
        >
          ...
          >
            ...
            <Select
              ...
              onChange={e => state.setPagination(p => ({ ...p, pageSize: Number(e.target.value), page: 1 }))}
            >
              {[10, 20, 50, 100].map(n => (
