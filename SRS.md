...
- **Core Responsibilities**:
  - Perform CRUD on items (unique IDs, categories, multiple copies with relations), scoped to tenants/instances.
  - Manage lending/returning workflows: Record user info, set due dates, track status, and trigger penalties/blacklists.
  - Handle reservations: Check availability, place holds, and resolve conflicts.
